  ...
  if (claims.length === 0) {
    return (
      <div className="rounded-2xl border border-dashed border-slate-200 py-20 text-center">
        ...
        <p className="mt-3 text-slate-500">대기 중인 소유권 요청이 없습니다.</p>
      </div>
    );
  }
      ...
      {claims.map((claim) => (
        ...
        >
          ...
          <div className="flex items-start gap-4 p-5">
            ...
            {claim.product?.url && (
              <a
                ...
                target="_blank"
                rel="noopener noreferrer"
                className="flex-shrink-0 rounded-xl border border-slate-200 px-3 py-1.5 text-xs font-medium text-slate-600 transition hover:border-blue-400"
              >
                🔗 방문
              </a>
            )}
          ...
          <div className="border-t border-slate-100 bg-slate-50 px-5 py-4">
            ...
            {claim.message && (
              <p className="mt-2 whitespace-pre-wrap rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm text-slate-700">
                {claim.message}
              </p>
            )}
          </div>
            ...
            {rejectingId === claim.id ? (
              <div className="space-y-2">
                <textarea
                  ...
                  className="w-full rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm text-slate-900 placeholder-slate-400 focus:border-red-300 focus:outline-none focus:ring-2 focus:ring-red-100"
                />
                <div className="flex items-center gap-2">
                  <button
                    type="button"
              ...
              <div className="flex items-center gap-2">
                <button
                  ...
                  className="rounded-lg bg-navy-900 px-4 py-2 text-xs font-semibold text-white transition hover:bg-navy-800 disabled:opacity-40"
                >
                ...
                <button
                  ...
                  className="rounded-lg border border-slate-200 px-4 py-2 text-xs font-semibold text-slate-600 transition hover:border-red-300 hover:text-red-600"
                >
                  반려
                </button>
              </div>
