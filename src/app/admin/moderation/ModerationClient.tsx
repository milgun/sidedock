  ...
  if (products.length === 0) {
    return (
      <div className="rounded-2xl border border-dashed border-slate-200 py-20 text-center">
        ...
        <p className="mt-3 text-slate-500">심사 대기 중인 제품이 없습니다.</p>
      </div>
    );
  }
        ...
        >
          ...
          <div className="flex items-start gap-4 p-5">
            ...
            <div className="min-w-0 flex-1">
              ...
              <p className="mt-0.5 text-sm text-slate-500">{product.tagline}</p>
              <div className="mt-2 flex flex-wrap gap-1">
                {product.categories.map((cat) => (
                  <span key={cat} className="rounded-full bg-slate-100 px-2 py-0.5 text-xs text-slate-500">
                    {cat}
                  </span>
                ))}
              </div>
            ...
            <div className="flex flex-shrink-0 items-center gap-2">
              <a
                ...
                target="_blank"
                rel="noopener noreferrer"
                className="rounded-xl border border-slate-200 px-3 py-1.5 text-xs font-medium text-slate-600 transition hover:border-blue-400"
              >
                🔗 방문
              </a>
            </div>
          ...
          {rejectingId === product.id && (
            <div className="border-t border-slate-100 px-5 py-4">
              ...
              <textarea
                ...
                className="w-full rounded-xl border border-slate-200 px-3.5 py-2.5 text-sm outline-none focus:border-red-400 focus:ring-2 focus:ring-red-100"
              />
              <div className="mt-3 flex gap-2">
                ...
                <button
                  ...
                  className="rounded-xl border border-slate-200 px-4 py-2 text-sm font-medium text-slate-600 transition hover:border-slate-400"
                >
                  취소
                </button>
              </div>
